...
これらの規約は、出し抜けに考えだされたものではない。これらの多くは、私のソフトウェアエンジニアとしての幅広い仕事や、私が携わってきた数多くのClojureオープソースプロジェクト footnote:[CIDER, nREPL, Orchard, etc.]、多くのClojureコミュニティメンバーからのフィードバックと意見、そして、 https://www.clojurebook.com/["Clojure Programming"] や https://joyofclojure.com/["The Joy of Clojure"] のような高い評価を受けている様々なリソースに基づいている。
...
この提案は確かにコミュニティ内で一定の支持を得ている一方で、Lispが培ってきた文化や、本スタイルガイドの主目的ーすなわち人間が読みやすいようコードを最適化するという点にも反している。
...
これらに共通するのは簡潔さを目指しつつも、エイリアスにある程度の意味を持たせようとしていることが明白だろう。（`clojure.walk` を `w` にエイリアスするのは簡潔ではあるが、ほとんど意味がない。）
...
BozhidarはClojureと優れたプログラミングスタイルに強い情熱を持っていて、https://clojure.org/community/contrib_howto#_coding_guidelines[Clojure library coding guidelines] の内容と、Java、Python、Rubyなどのプログラミング言語のスタイルガイドに書かれている典型的な内容のギャップを埋めることを目指していました。現在も Bozhidar はこのガイドの主任編集者ですが、プロジェクトを支える編集チームが存在しています。
