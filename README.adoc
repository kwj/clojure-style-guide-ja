...
image:http://i.creativecommons.org/l/by/3.0/88x31.png[Creative Commons License] 本翻訳版のライセンスは https://creativecommons.org/licenses/by/3.0/deed.ja[クリエイティブ・コモンズ 表示 3.0 非移植 ライセンス] とします。原著作者はBozhidar Batsov氏です。原文のライセンスについてもご注意ください。
...
これらの規約は、出し抜けに考えだされたものではない。これらの多くはスタイルガイドの編集者たちの経験に基づいており、多くのClojureコミュニティメンバーからのフィードバックと意見、そして、 http://www.clojurebook.com/["Clojure Programming"] や https://www.manning.com/books/the-joy-of-clojure-second-edition["The Joy of Clojure"] のような高い評価を受けている様々なリソースに基づいている。

このスタイルガイドはまだまだ書き換えることができる。新たな慣習が確認されたり、Clojure自体の変更によって従来の慣習が時代遅れになったりすることに伴い、このスタイルガイドは時間とともに変化する。

NOTE: Clojure開発コミュニティはhttps://clojure.org/community/contrib_howto#_coding_guidelines[ライブラリのコーディング規約]footnote:[これらのガイドラインはClojure自身およびClojureのContribライブラリに適用することを意図しています。]の一覧もまとめている。これらは、あなたが現在読んでいるドキュメントの着想源の一つとなりました。
...
単一セグメントの名前空間を避けるべき https://github.com/bbatsov/clojure-style-guide/pull/100[理由は他にもあり]、それらを利用する前によくよく検討すべきだ。
...
=== 慣用的な名前空間エイリアスを使う [[use-idiomatic-namespace-aliases]]
...
=== 良い名前空間エイリアスのためのレシピ [[a-recipe-for-good-namespace-aliases]]
...
しかし、慣用的なエイリアスを持たない他の名前空間についてはどうすればよいのだろうか？ エイリアス名を決める際には一貫したアプローチをとるべきだ、そうでないとClojureのコードを共有して作業している他の人たちは大きな混乱に陥るだろう。下記に従うべきいくつかのルールを示す。 footnote:[このガイドラインはStuart Sieraによるhttps://stuartsierra.com/2015/05/10/clojure-namespace-aliases[ブログ記事]に基づいている。]
...
=== 一貫した名前空間エイリアスの使用 [[use-consistent-namespace-aliases]]
...
=== 名前空間の命名方法 [[naming-ns-naming-schemas]]
...
=== 名前空間セグメントにおける複数単語 [[naming-namespace-composite-segments]]
...
=== 関数、変数 [[naming-functions-and-variables]]
...
=== プロトコル、レコード、構造体、型 [[naming-protocols-records-structs-and-types]]
...
=== 安全でない関数 [[naming-unsafe-functions]]
...
=== 変換のための関数 [[naming-conversion-functions]]
...
=== DynamicなVar [[naming-dynamic-vars]]
...
=== 使用しない束縛 [[naming-unused-bindings]]
...
=== マルチメソッドのディスパッチの位置 [[multimethod-dispatch-val-placement]]
...
=== 複数アリティのインデント [[multiple-arity-indentation]]
...
=== ブール値への変換 [[converting-something-to-boolean]]
...
=== 柔軟な比較関数 [[flexible-comparison-functions]]
...
=== 無名関数 vs `complement`, `comp`, `partial` [[anonymous-functions-vs-complement-comp-and-partial]]

`complement`、`comp`、`partial` より無名関数を使うことを薦める。ほとんどの場合、そうすることでコードが単純になる。 footnote:[この件に関する詳細はhttps://ask.clojure.org/index.php/8373/when-should-prefer-comp-and-partial-to-anonymous-functions[こちら]をご覧ください。]
...
=== スレッディングマクロと省略可能な括弧 [[threading-macros-and-optional-parentheses]]
...
=== スレッディングマクロの揃え方 [[threading-macros-alignment]]
...
=== Java呼び出しよりも `clojure.math` の関数 [[prefer-clojure-math-over-interop]]
...
=== Java呼び出しよりも `clojure.string` の関数 [[prefer-clojure-string-over-interop]]
...
=== Throwable のキャッチ [[catching-throwables]]
...
https://github.com/weavejester/codox#metadata-options[Codox] や https://github.com/cljdoc/cljdoc/blob/master/doc/userguide/for-library-a
...
=== インデント用メタデータ [[indentation-metadata]]

他のLisp方言とは異なり、Clojureはマクロのインデントを指定する標準的なメタデータは存在しない。2015年、CIDERはメタデータに基づいたツールに依存しない https://docs.cider.mx/cider/indent_spec.html[インデントの指定方法] を提案した。　footnote:[これはCIDER 1.0で初めて導入されました]簡単な例をいくつか示す：
...
NOTE: このインデントの考え方（セマンティック・インデント）は、メタデータの追加とツールのサポートが必要となるためClojureコミュニティで議論の的になっている。この考え方はLispコミュニティで長い伝統があるが、一部の人たちは関数とマクロの区別をやめて、すべてを統一した固定インデントにすべきだと主張している。 https://tonsky.me/blog/clojurefmt/[この記事]はそのような主張に関する代表的な資料の一つである。
...
=== 自己説明的なコード [[self-explanatory-code]]

出来る限り、コードの役割や意図を一目でわかるように努める。これができない場合は、このセクションの残りのガイドラインに従うこと。

=== 見出しコメント [[four-semicolons-for-heading-comments]]
...
=== トップレベルのコメント [[three-semicolons-for-top-level-comments]]
...
=== コード部分（行）コメント [[two-semicolons-for-code-fragment]]
...
=== 行末（インライン）コメント [[one-semicolon-for-margin-comments]]
...
=== ドキュメント文字列は関数名の後ろ [[docstring-after-fn-name]]
...
=== コンテキストのために `testing` ブロック [[use-testing-blocks]]
...
=== 情報提供型のアサーションメッセージが好ましい [[informative-assertion-messages]]
...
=== テーブル駆動テストには `are` [[use-are-for-tabular-tests]]
...
=== `deftest` ごとに1つのコンセプトでテスト [[one-concept-per-deftest]]

それぞれの `deftest` は1つの論理的な挙動をカバーするべきだ。
各エッジケースについては `deftest` を分割するのではなく、`testing` を使ってサブケースとしてまとめる。

=== `with-redefs` の使用は控えめに [[use-with-redefs-sparingly]]
...
=== 例外のテスト [[test-expected-exceptions]]
...
=== 構造的なデータに対するアサーションには `match?` [[prefer-match-for-structural-assertions]]

生成されたフィールドや動的に値が変化するフィールド（IDやタイムスタンプ）を含むマップを返す関数をテストする場合は、
https://github.com/nubank/matcher-combinators[matcher-combinators]に含まれる `match?`の使用を検討する。
...
=== 例外データに対しては `thrown-match?` [[use-thrown-match-for-exception-data]]
...
* https://github.com/kkinnear/zprint[zprint] （コミュニティのフォーマットルールを使用するよう設定するためのドキュメントは https://github.com/kkinnear/zprint/blob/master/doc/options/community.md[ここ]にあります。）

NOTE: エディタについて、Emacsの `clojure-mode` はデフォルトでスタイルガイドに記載されているとおりにコードをフォーマットする。他のエディタについては、同じ結果を得るために設定を調整する必要があるかもしれない。

== 歴史 [[history]]

このスタイルガイドは https://github.com/bbatsov[Bozhidar Batsov] により、彼がRubyコミュニティで立ち上げた　https://rubystyle.guide/[類似プロジェクト] の成功を受けて2013年に開始されました。

BozhidarはClojureと優れたプログラミングスタイルに強い情熱を持っていて、https://clojure.org/community/contrib_howto#_coding_guidelines[Clojure library coding guidelines] の内容と、Java、Python、Rubyなどのプログラミング言語のスタイルガイドで通常扱われている内容とのギャップを埋めることを目指していました。現在も Bozhidar はこのガイドの主任編集者ですが、プロジェクトを支える編集チームが存在しています。
...
== どのように貢献すればよいですか？ [[how-to-contribute]]

簡単です、以下は貢献のためのガイドラインです：

* リポジトリ https://github.com/bbatsov/clojure-style-guide[bbatsov/clojure-style-guide] を GitHub 上で https://help.github.com/articles/fork-a-repo[フォーク] します。
* 内容の追加や誤りの修正は新しいブランチを切っておこなってください。
* 変更内容について http://tbaggery.com/2008/04/19/a-note-about-git-commit-messages.html[詳しい説明をつけて] コミットします。
* 変更したブランチをGitHubにプッシュします。
* https://help.github.com/articles/using-pull-requests[Pull Requestを送って] ください。

== 奥付 [[colophon]]

このガイドはhttps://asciidoc.org/[AsciiDoc]で記述されており、https://asciidoctor.org/[AsciiDoctor]でHTML形式で公開されています。HTML版はGitHub Pagesで提供されています。
