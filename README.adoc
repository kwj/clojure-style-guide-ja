...
この提案は確かにコミュニティ内で一定の支持を得ている一方で、Lispが培ってきた文化や、本スタイルガイドの主目的 -- すなわち人間が読みやすいようコードを最適化するという点にも反している。

固定インデントにはほとんど話題に上がらない小さな注意点があり、それはリストリテラルのインデント方法である（関数呼び出しは単なるリストリテラルです）。Clojureのコード自体の構造を定義する文脈以外ではリストリテラルは一般的ではなく、通常、この問題点は考慮の対象から外れている。
...
見ての通り少し奇妙であるし、他のコレクション型の通常のインデントと一貫性がとれていない。
